...
class DomainRelated_Feature_Selection(nn.Module):
    def __init__(self, num_channels=256):
        ...
        self.theta1 = nn.Parameter(torch.zeros(1, num_channels, 1, 1)).to(device)
        self.theta2 = nn.Parameter(torch.zeros(1, num_channels * 2, 1, 1)).to(device)
        self.theta3 = nn.Parameter(torch.zeros(1, num_channels * 4, 1, 1)).to(device)
